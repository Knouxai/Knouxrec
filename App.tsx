...
const App = () => {
  ...
  const runAiProcessing = useCallback(async (recording: Recording) => {
    if (!recording.transcript || recording.transcript.trim().length < 10) {
      addNotification(`AI analysis skipped for "${recording.name}" (transcript too short).`, 'info');
      setRecordings(prev => prev.map(r => r.id === recording.id ? { ...r, isProcessing: false, summary: 'No significant speech detected.' } : r));
      return;
    }

    try {
      addNotification(`Processing AI analysis for "${recording.name}"...`, 'info');
      const result = await processTranscript(recording.transcript);
      setRecordings(prev => prev.map(r =>
        r.id === recording.id
          ? { ...r, name: result.title, summary: result.summary, keywords: result.keywords, isProcessing: false }
          : r
      ));
      addNotification(`AI analysis complete for "${result.title}"`, 'success');
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : "An unknown error occurred";
      addNotification(`AI analysis failed: ${errorMessage}`, 'error');
      setRecordings(prev => prev.map(r => r.id === recording.id ? { ...r, isProcessing: false, summary: `AI Analysis Failed: ${errorMessage}` } : r));
    }
  }, [addNotification]);

  const onRecordingComplete = useCallback((blob: Blob, transcript: string) => {
    const newRecording: Recording = {
      ...
      size: blob.size,
      date: new Date(),
      duration: 0, // Duration should be calculated by the hook
      transcript,
  ...
  useEffect(() => {
    const checkHotkey = (e: KeyboardEvent, hotkey: string): boolean => {
      ...
      return e.key.toLowerCase() === key.toLowerCase() && e.ctrlKey === ctrl && e.shiftKey === shift && e.altKey === alt && e.metaKey === meta;
    };

    const handleHotkeys = (e: KeyboardEvent) => {
      const target = e.target as HTMLElement;
      if (['INPUT', 'TEXTAREA', 'SELECT'].includes(target.tagName) || target.isContentEditable || isSettingsOpen || pendingRecording) {
        return;
      }

      if (checkHotkey(e, settings.hotkeys.startStop)) {
        e.preventDefault();
        recorder.recordingState === 'IDLE' ? recorder.startRecording() : recorder.stopRecording();
      } else if (checkHotkey(e, settings.hotkeys.pauseResume)) {
        e.preventDefault();
        if (recorder.recordingState === 'RECORDING') recorder.pauseRecording();
        else if (recorder.recordingState === 'PAUSED') recorder.resumeRecording();
  ...
  const handleStartRecording = () => {
    setGalleryPlaybackUrl(null); // Clear gallery preview before starting
